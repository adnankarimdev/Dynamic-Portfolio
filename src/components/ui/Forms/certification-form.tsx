...
export function CertificationForm({ onSubmit, editingItem }: CertificationFormProps) {
  ...
  const handleDateChange = (date: Date | undefined) => {
    if (date) {
      setFormData((prev) => ({ ...prev, dateIssued: format(date, "yyyy-MM-dd") }))
    }
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        ...
        <Input id="issuingOrganization" name="issuingOrganization" value={formData.issuingOrganization} onChange={handleChange} required />
      </div>
      <div>
        ...
        <Popover>
          <PopoverTrigger asChild>
            ...
            >
              ...
              {formData.dateIssued ? format(new Date(formData.dateIssued), "PPP") : <span>Pick a date</span>}
            </Button>
          </PopoverTrigger>
          <PopoverContent className="w-auto p-0">
            <Calendar
              mode="single"
              selected={formData.dateIssued ? new Date(formData.dateIssued) : undefined}
      ...
      <Button type="submit">{editingItem ? "Update Certification" : "Add Certification"}</Button>
    </form>
  )
}
