...
export function AwardForm({ onSubmit }: AwardFormProps) {
  ...
  const handleDateChange = (date: Date | undefined) => {
    if (date) {
      setFormData((prev) => ({ ...prev, dateAwarded: format(date, "yyyy-MM-dd") }))
    }
  }
        ...
        <Popover>
          <PopoverTrigger asChild>
            ...
            >
              ...
              {formData.dateAwarded ? format(new Date(formData.dateAwarded), "PPP") : <span>Pick a date</span>}
            </Button>
          </PopoverTrigger>
          <PopoverContent className="w-auto p-0">
            <Calendar
              mode="single"
              selected={formData.dateAwarded ? new Date(formData.dateAwarded) : undefined}
