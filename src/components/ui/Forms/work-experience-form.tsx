...
export function WorkExperienceForm({ onSubmit, editingItem }: WorkExperienceFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        ...
        <Input id="href" name="href" type="url" value={formData.href} onChange={handleChange} />
      </div>
      <div>
        ...
        <Input
          ...
          onChange={(e) => setFormData((prev) => ({ ...prev, badges: e.target.value.split(", ") }))}
        />
      </div>
      <div>
        ...
        <Popover>
          <PopoverTrigger asChild>
            ...
            >
              ...
              {formData.start ? format(new Date(formData.start), "PPP") : <span>Pick a date</span>}
            </Button>
          </PopoverTrigger>
      ...
      <div className="space-y-2">
        ...
        <Popover>
          <PopoverTrigger asChild>
            ...
            >
              <CalendarIcon className="mr-2 h-4 w-4" />
              {formData.end ? format(new Date(formData.end), "PPP") : <span>Pick a date</span>}
            </Button>
          </PopoverTrigger>
          <PopoverContent className="w-auto p-0">
            <Calendar
      ...
      <Button type="submit">{editingItem ? "Update Work Experience" : "Add Work Experience"}</Button>
    </form>
  )
}
