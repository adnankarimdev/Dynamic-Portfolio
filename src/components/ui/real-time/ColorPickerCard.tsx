...
export default function ColorPickerCard({ cardData }: ColorCardProps) {
  ...
  const isDarkColor = [
    ...
    "Forest",
    "Kelly",
  ].includes(selectedColor.name);

  return (
    <AnimatedLayout>
      <Card
        className={`w-full p-20 transition-colors duration-300 ${spaceMono.className}`}
        style={{ backgroundColor: cardData.color }}
      >
        <CardHeader>
          <CardTitle className={`text-xl mb-4 ${cardData.textColor}`}>
            <LetterPullup
              words={cardData.title}
              delay={0.05}
              className={`text-sm ${cardData.textColor}`}
            />
          </CardTitle>
        </CardHeader>
        <BoxReveal boxColor={"#5046e6"} duration={0.5}>
          <CardContent>{cardData.summary}</CardContent>
        </BoxReveal>
      </Card>
    </AnimatedLayout>
  );
